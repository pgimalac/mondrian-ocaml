...
let rec head l = match l with a :: b :: q -> a :: (b :: q |> head) | _ -> []
...
module Make_Colors (S : Game_settings) = struct
  let nb_colors = match S.color with RGColor -> 2 | RGBColor -> 3

  let game_colors =
    match S.color with RGBColor -> game_colors | RGColor -> head game_colors

  let index color =
    let rec iter acc l =
      match l with
      | h :: q -> if h = color then Some acc else iter (acc + 1) q
      | [] -> None
    in
    iter 0 game_colors
  ...
  let next_color reverse c =
    ...
    in
    (if reverse then aux_r else aux) game_colors

  let rand_color () = List.nth game_colors (Random.int nb_colors + 1)
end
