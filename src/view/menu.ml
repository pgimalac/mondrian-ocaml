...
let game_mode_buttons, show_game_mode_buttons =
  buttons_groups
    [
      ( create_button "Classic mode" 180. 350. 110 75,
        fun () -> game_mode := Classic );
      ( create_button "Extrem mode" 310. 350. 110 75,
        fun () -> game_mode := Extrem );
    ]

let game_color_buttons, show_game_color_buttons =
  buttons_groups
    [
      ( create_button "2 Colors" 180. 450. 110 75,
        fun () -> color_mode := RGColor );
      ( create_button "3 Colors" 310. 450. 110 75,
        fun () -> color_mode := RGBColor );
    ]

let buttons = [ (create_button "Play" 175. 75. 250 75, fun () -> ()) ]
...
let slide_bar title y min max value st_opt =
  ...
  let _ =
    ...
    | Some st ->
        if
          st.button && st.mouse_x >= slide_bar_x && st.mouse_x <= slide_bar_xx
          && st.mouse_y >= y - slide_bar_padding
          && st.mouse_y <= y + slide_bar_padding
        then
          value :=
            ((st.mouse_x - slide_bar_x) * (max - min) / slide_bar_width) + min
  in

  set_line_width 2;
  let x = ((!value - min) * slide_bar_width / (max - min)) + slide_bar_x in
  moveto x (y - 10);
  lineto x (y + 10);
...
let show_buttons st_opt =
  let hover_predicate =
    ...
    | _ -> fun _ -> false
  in
  List.iter (fun (btn, _) -> print_btn ~hover:(hover_predicate btn) btn) buttons
...
let show_menu st =
  ...
  slide_bar "Black probability" 275 min_black max_black black_probability
    (Some st);
  show_game_mode_buttons ();
  show_game_color_buttons ();
...
let menu = { plot = show_menu; handler = select_mode }
